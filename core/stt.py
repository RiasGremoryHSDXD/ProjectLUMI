    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            self.recorder = AudioToTextRecorder(
                ...
                language="en",
                device="cpu",  # Switched to CPU to avoid 'cublas64_12.dll not found' errors on Windows
                spinner=False,  # Disable spinner for cleaner output
                # wakeword_backend="pvporcupine",  # REMOVED: Incompatible with custom wake word "lumi" without key
                # wake_words=WAKE_WORD,  # REMOVED: disabling built-in to handle manually for "lumi"
        ...
        try:
            ...
            while self.running:
                ...
                if text and text.strip():
                    # Manual Wake Word Detection
                    # Check if wake word is present in the transcribed text
                    if WAKE_WORD in text.lower():
                        print(f"{CYAN}[STT] 👂 Wake word '{WAKE_WORD}' detected in stream!{RESET}")
                        
                        # Remove wake word from the text
                        # We use regex to handle case insensitive replacement safely if needed, or just string replace
                        import re
                        # Case insensitive remove of wake word
                        text_clean = re.sub(re.escape(WAKE_WORD), '', text, flags=re.IGNORECASE).strip()
                        
                        print(f"{CYAN}[STT] 🧹 Cleaned text: '{text_clean}'{RESET}")
                        
                        if text_clean:
                            print(f"{CYAN}[STT] 🔊 Command recognized: '{text_clean}'{RESET}")
                            # Pass transcribed speech to callback
                            self.speech_callback(text_clean)
                        else:
                            print(f"{GRAY}[STT] ⚠ Wake word detected but no command follows.{RESET}")
                    else:
                        print(f"{GRAY}[STT] 🔇 Ignoring speech (no wake word): '{text}'{RESET}")
                else:
